...
##
## A service is only registered (and automatically started with vagent) if it is
## expected to run on the node invoking this function.
##
## Input:
##      SRVCID      -> Id of the service to register
##      REG_OPTION  -> Command line argument to bd_vcli.
## Return:
...
REGISTER_START_SERVICE_SYSV() {
    ...
    REGISTER_START_SERVICE_COMMON "${SRVCID}" "--systemv=${SYSV}"
}
